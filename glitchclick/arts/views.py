...
def recomendations(request):
	...
	if profile.are_recomendations_are_ready > 15:
		...
		gender = None
		arts = []
		if quer_girl_boy.split().count("boy")*3 <= quer_girl_boy.split().count("girl"):
			gender = "girl"
			arts = list(arts)
			query = query.split()
			for i in range(len(query)):
				searched = Art.objects.filter(Q(keywords__icontains=query[i].lower()) & Q(gender__icontains=gender))
				for j in range(len(searched)):
					if searched[j] not in profile.favorites.all():
						arts.append(searched[j])
		elif quer_girl_boy.split().count("girl")*3 <= quer_girl_boy.split().count("boy"):
			gender = "boy"
			arts = list(arts)
			query = query.split()
			for i in range(len(query)):
				...
				for j in range(len(searched)):
					if searched[j] not in profile.favorites.all():
						arts.append(searched[j])
		elif quer_girl_boy.split().count("girl") >= quer_girl_boy.split().count("boy") and quer_girl_boy.split().count("girl") < quer_girl_boy.split().count("boy")*3:
			arts = list(arts)
			query = query.split()
			for i in range(len(query)):
				searched = Art.objects.filter(Q(keywords__icontains=query[i].lower()))
				for j in range(len(searched)):
					if searched[j] not in profile.favorites.all():
						arts.append(searched[j])
		elif quer_girl_boy.split().count("boy") >= quer_girl_boy.split().count("girl") and quer_girl_boy.split().count("boy") < quer_girl_boy.split().count("girl")*3:
			arts = list(arts)
			query = query.split()
			for i in range(len(query)):
				searched = Art.objects.filter(Q(keywords__icontains=query[i].lower()))
